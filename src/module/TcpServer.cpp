...
void TcpServer::Accept()
{
    ...
    PrintDbg(DBG_PRT, "%s:Incomming...", name.c_str());
    ...
    {
        close(connfd);
        PrintDbg(DBG_LOG, "%s:Connection pool is full, reject", name.c_str());
        return;
    }
    ...
    PrintDbg(DBG_PRT, "%s:Accept %s as %s", name.c_str(), ip_port, tcpOperator->Name().c_str());
}
