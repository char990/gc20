...
void OprTcp::EventsHandle(uint32_t events)
{
    ...
    {
        Pdebug("%s-%s:Disconnected:events=0x%08X", name.c_str(), client, events);
        Release();
    }
...
long OprTcp::IdleMs()
{
    long s = DbHelper::Instance().GetUciUser().SessionTimeoutSec();
...
int OprTcp::Tx(uint8_t *data, int len)
{
    ...
    {
        Ldebug("%s-%s:Tx failed", name.c_str(), client);
        Release();
    }
    return (x < 10 ? 10 : x);
}
...
void OprTcp::PeriodicRun()
{
    ...
    {
        Pdebug("%s-%s:Idle timeout. Disconnected", name.c_str(), client);
        Release();
    }
}
    ...
    while (1)
    {
        ...
        else
        {
            ...
            { // if tx is busy, discard this rx
                Pdebug("%s-%s:Tx not ready, discard rx", name.c_str(), client);
            }
        }
    }
}
